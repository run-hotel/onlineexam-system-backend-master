...
import com.mwt.oes.service.TeacherFeedbackService;
...
public class TeacherFeedbackController {
    ...
    public ServerResponse getFeedbacksList() {
        List<Map<String, Object>> resultList = teacherFeedbackService.getFeedbacksList();
    ...
    public ServerResponse getUnReplyCount() {
        int result = teacherFeedbackService.getUnReplyCount();
    ...
    public ServerResponse searchFeedbacksList(@RequestParam("feedbackContent") String feedbackContent,
        ...
        List<Map<String, Object>> resultList = teacherFeedbackService.searchFeedbacksList(feedbackContent, stuName, admAnswer, admName, feedbackStatus);
        return ServerResponse.createBySuccess("获取全部轮播图信息成功", resultList);
    }
    ...
    public ServerResponse deleteFeedback(@RequestBody Map<String, Object> obj) {
        ...
        int result = teacherFeedbackService.deleteFeedback(feedbackId);
        if (result > 0) {
    ...
    public ServerResponse replyFeedback(@RequestBody(required = false) StudentFeedback studentFeedback) throws IOException {
        int result = teacherFeedbackService.replyFeedback(studentFeedback);
        if (result > 0) {
